...
async def test_marian_unavailable_falls_back_to_llm(monkeypatch):
    from buddle.ai.news import marian
    ...
    async def fake_ai(_messages, *, settings, json_mode, max_tokens):  # type: ignore[no-untyped-def]
        return json.dumps({"items": [{"i": 0, "title_ko": "폴백 번역 제목", "summary_ko": "요약"}]})

    monkeypatch.setattr(marian, "translate_batch", none_batch)
    monkeypatch.setattr(tr, "_call_ai", fake_ai)
    out = await tr.translate_articles([_art("Fallback path check")], settings=_Settings("marian"))
    assert out[0].translated and out[0].title == "폴백 번역 제목"


async def test_all_engines_fail_open_keeps_original(monkeypatch):
    from buddle.ai.news import marian

    async def none_batch(_texts, *, model_name=None):  # type: ignore[no-untyped-def]
        return None

    async def dead_ai(*_a, **_kw):  # type: ignore[no-untyped-def]
        return None

    monkeypatch.setattr(marian, "translate_batch", none_batch)
    monkeypatch.setattr(tr, "_call_ai", dead_ai)
    a = _art("Original stays intact")
    out = await tr.translate_articles([a], settings=_Settings("marian"))
    assert not out[0].translated and out[0].title == a.title  # fail-open
